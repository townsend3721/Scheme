...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        evaluate=operands.map(frame)
        return scheme_apply(self,evaluate,env)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    statement = LambdaProcedure(formals, expressions.second, env)
    return statement
    # END PROBLEM 9
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        if not scheme_symbolp(bindings.first.first):
            raise SchemeError("bindings does not begin with a symbol")
